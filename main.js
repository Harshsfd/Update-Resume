...
document.addEventListener('DOMContentLoaded', () => {
  const page = document.getElementById('page') || document.getElementById('resume');
  if (!page) return;

  // Autosave to LocalStorage
  const KEY = 'siet_resume_content_v1';
  try {
  ...
  let t;
  const saveNow = () => {
    try { localStorage.setItem(KEY, page.innerHTML); } catch (e) {}
  };
  const debouncedSave = () => { clearTimeout(t); t = setTimeout(saveNow, 500); };

  page.addEventListener('input', debouncedSave);
  page.addEventListener('blur', saveNow);

  // Print
  const btnPrint = document.getElementById('btnPrint');
  if (btnPrint) btnPrint.addEventListener('click', () => window.print());

  // Export ATS-friendly TXT
